        ...
        try {
          ...
          if (dslPlatformIds.length > 0) {
            await syncToMultiplePlatforms(dslPlatformIds, article, {
              ...
              onDetailProgress: (progress) => {
                sendToEditor({ type: 'SYNC_DETAIL_PROGRESS', progress })
              },
            }, syncId)
          }
          ...
          if (cmsPlatformIds.length > 0) {
            for (const accountId of cmsPlatformIds) {
              ...
              sendToEditor({ type: 'SYNC_DETAIL_PROGRESS', progress: { platform: accountId, platformName: account.name, stage: 'starting' } })
              ...
              try {
                ...
                sendToEditor({ type: 'SYNC_DETAIL_PROGRESS', progress: { platform: accountId, platformName: account.name, stage: 'saving' } })
          ...
          // 更新历史记录
          await updateHistoryItem(syncId, 'completed', allResults, allPlatformMetas)
